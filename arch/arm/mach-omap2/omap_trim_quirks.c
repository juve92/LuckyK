/*
 * OMAP trim quirks control and configuration
 *
...
#define OMAP4_DPLL_MPU_TRIMMED_MASK	(0x3 << 18)
...
/**
 * omap_trim_configure() - Handle device trim variance
 *
 ...
 */
int omap_trim_configure(void)
{
	if (cpu_is_omap44xx())
		return omap4_trim_configure();
...
static __init int omap4_trim_init(void)
{
	u32 bgap_trimmed = 0;
	...
	return omap_trim_configure();
}

static __init int omap_trim_init(void)
{
	if (cpu_is_omap44xx())
		return omap4_trim_init();

	return 0;
}
arch_initcall(omap_trim_init);
